...
def main(src_dst_list):
        ...
        if response_zone_entries['zone_entries']['total'] > 0:
            for entry in response_zone_entries['zone_entries']['zone_entry']:
                ...
                new_entry = {'id': entry['zoneId'], 'name': entry['zoneName'], 'entry_id': entry['id'],
                             'network': ipv4_network, 'netmask': entry['netmask'], 'prefix': entry['prefix'], 'domain': entry['domain']}
            ...
            for src_net in src_networks:
                ...
                if len(src_zone_entry) > 0:
                    for usp_zone_check in src_zone_entry:
                        if len(response_get_usp_df[(response_get_usp_df['from zone'] == usp_zone_check[0])].values.tolist()) > 0:
            ...
            for dst_net in dst_networks:
                ...
                if len(dst_zone_entry) > 0:
                    for usp_zone_check in dst_zone_entry:
                        if len(response_get_usp_df[(response_get_usp_df['to zone'] == usp_zone_check[0])].values.tolist()) > 0:
    ...
    f = open("/etc/ansible/awx_playbooks/ansible_secops_test/ansible_python/usp_cleared.py", "a", encoding="UTF8")
    # f = open("E:\\Downloads\\ansible_secops_test\\ansible_secops_test\\ansible_python\\usp_cleared.py", "w", encoding="UTF8")

    f.write("def src_ip_addresses():" + "\n")
    f.write(f"   src_ip_addresses = {src_ip_addresses} \n")
    f.write(f"   return src_ip_addresses \n\n")

    f.write("def dst_ip_addresses():" + "\n")
    f.write(f"   dst_ip_addresses = {dst_ip_addresses} \n")
    f.write(f"   return dst_ip_addresses \n\n")

    f.write("def src_dst_passed():" + "\n")
    f.write(f"   src_dst_passed = {src_dst_passed} \n")
    f.write(f"   return src_dst_passed \n\n")

    f.write("def access_role_list():" + "\n")
    f.write(f"   access_role_list = {access_role_list} \n")
    f.write(f"   return access_role_list")

    f.close()
    
    return src_ip_addresses, dst_ip_addresses, src_dst_passed, access_role_list   

exec(open("/etc/ansible/awx_playbooks/ansible_secops_test/ansible_python/initial_request.py").read())
# exec(open("E:\\Downloads\\ansible_secops_test\\ansible_secops_test\\ansible_python\\initial_request.py", encoding="UTF8").read())
